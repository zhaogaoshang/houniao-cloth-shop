<template>
  <div class="public-column category">
    <user>
      ...
      <discolor-btn typeStyle="middle" @click.native="handelAdd">添加部位</discolor-btn>
    </user>
    <div class="public-row bottom-info">
      <shadow-box>
        <div class="title">量体部位管理</div>
        <div>
          <el-table
...
export default {
  ...
  */
  ...
  methods: {
    ...
    // 获取列表
    _getList () {
      this.$http.post(this.$apis.api_volumeBodily_list, this.params).then(res => {
        if (res.code !== 'SUCCESS') return this.$message(res.msg)
